"""CLI entry point: python -m server.ml.train [--symbol SYM] [--backtest] [--lstm]"""

import argparse
import time

from server.database import get_conn
from server.ml.model import train
from server.ml.backtest import run_backtest
...
def main():
    ...
    for sym in symbols:
        ...
        if args.lstm and sym in LSTM_CONFIGS:
            from server.ml.lstm_model import train_and_save_lstm
