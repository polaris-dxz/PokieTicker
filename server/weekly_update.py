...
Run manually or via cron: python -m server.weekly_update

Cron example (every Sunday at 2am):
  0 2 * * 0 cd /path/to/PokieTicker && python -m server.weekly_update >> logs/weekly.log 2>&1
"""
...
from server.polygon.client import fetch_ohlc, fetch_news, http_get, BASE
...
def main():
    ...
    print(f"\nNote: Run 'python -m server.batch_submit' to process new articles through Layer 1")
