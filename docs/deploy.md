...
- **前后端合一**：构建后的 UI（`app/dist`）由 FastAPI 托管，访问根路径会重定向到 `/PokieTicker/`，API 在 `/api/*`。
- **数据库**：使用项目自带的 SQLite（`.data/pokieticker.db`）。若仓库里有 `pokieticker.db.gz` 或 `.data/pokieticker.db.gz`，在构建阶段解压到 `.data/` 即可；若无，需自行准备或接受「无数据」仅演示界面。
...
- 代码在 **GitHub** 上。
- 若需预置数据：仓库内包含 `pokieticker.db.gz` 或 `.data/pokieticker.db.gz`（或构建时解压到 `.data/pokieticker.db`）。
...
(test -f .data/pokieticker.db.gz && gunzip -k .data/pokieticker.db.gz) || (test -f pokieticker.db.gz && gunzip -k -c pokieticker.db.gz > .data/pokieticker.db) || true
```
...
- 每月 750 小时，单实例够用。
- 磁盘不持久：每次重新部署会清空本地文件，数据库需来自构建（如解压到 `.data/pokieticker.db`）或外部存储（需自行接 S3 等）。
...
COPY server/ ./server/
COPY --from=app-build /app/app/dist ./app/dist
COPY .env* ./
RUN mkdir -p .data && (test -f pokieticker.db.gz && gunzip -k -c pokieticker.db.gz > .data/pokieticker.db || true)

ENV PORT=8000
EXPOSE 8000
CMD uvicorn server.api.main:app --host 0.0.0.0 --port ${PORT}
```

- 构建时会把 App 打进镜像，并解压数据库到 `.data/pokieticker.db`（若存在 `pokieticker.db.gz`）。
...
3. 在 **fly.toml** 中确保有：
   ...
   - 启动命令用：`uvicorn server.api.main:app --host 0.0.0.0 --port 8080`（与 PORT 一致）。
4. 构建与数据库：可用 Dockerfile（同上），或在 **Dockerfile** 里用多阶段构建，把 `app/dist` 和（可选）解压后的 `.data/pokieticker.db` 打进镜像。
5. 部署：`fly deploy`。
...
2. 构建：  
   `pip install -r requirements.txt`，`cd app && pnpm install && pnpm run build`，必要时解压数据库到 `.data/pokieticker.db`。
...
- **打开页面空白或 404**  
  确认构建产物在 `app/dist`，且 server 已挂载静态资源（见 `server/api/main.py` 中对 `app/dist` 的挂载）。访问路径应为 **/PokieTicker/**（带尾部斜杠）。

- **API 404 / 预测不可用**  
  预测接口依赖训练好的模型（见 [原理与流程](overview.md)）。免费部署通常不包含模型文件，需在本地跑 `python -m server.ml.train` 后把 `.data/models/` 打进镜像或部署包，或接受「暂无预测」的提示。
